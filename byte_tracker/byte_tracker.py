...
class STrack(BaseTrack):
    ...
    def tlbr(self):
        ...
        ret = self.tlwh.copy()
        ret[2:] += ret[:2]
        '''bbox0 = self.tlwh.copy()
        cx = (bbox0[0]+bbox0[2])/2
        cy = (bbox0[1]+bbox0[3])/2
        hw = self._tlwh[2]/2
        hh = self._tlwh[3]/2
        ret = np.array([cx-hw,cy-hh,cx+hw,cy+hh],dtype=np.float32)'''
        #ret[2:] += ret[:2]
        return ret
...
class BYTETracker(object):
    def __init__(self, det_thresh=0.2,frame_rate=30,track_buffer=30,assignment_thresh=[0.9,0.9,0.7]):
    ...
    def apply(self,bboxes,probs,is_first_frame=False):
        img_info = (100,100)
        img_size = (100,100)
        if is_first_frame:
            BaseTrack._count = 0
            self.frame_id = 0
        return self.update(bboxes,probs,img_info,img_size)

    def update(self, bboxes,probs, img_info, img_size):
        self.frame_id += 1
        activated_starcks = []
        refind_stracks = []
        lost_stracks = []
        ...
        all_indexs = np.array(list(range(scores.shape[0])),dtype=np.int32)
        ...
        dets_idx = all_indexs[remain_inds]
        scores_keep = scores[remain_inds]
        scores_second = scores[inds_second]
        idx_second = all_indexs[inds_second]
        ...
        if len(dets_second) > 0:
            ...
            for i,det in enumerate(detections_second):
                det.set_track_idx(idx_second[i])
        else:
            detections_second = []
